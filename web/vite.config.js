  ...
  const config = {
    ...
    server: {
      ...
      proxy: {
        ...
        [process.env.VITE_BONUS_API]: {
          target: `${process.env.VITE_BONUS_PATH}:${process.env.VITE_BONUS_PORT}/`, // 代理到 目标路径
          changeOrigin: true,
          rewrite: path => path.replace(new RegExp('^' + process.env.VITE_BONUS_API), ''),
        }
      },
    },
    build: {
