      ...
      <el-form ref="elFormRef" :model="formData" :rules="rule" label-position="right" label-width="200px">
        ...
        <el-form-item label="充电桩类型:" prop="pileType">
          <el-select v-model="formData.pileType" :clearable="true" placeholder="请选择" style="width:100%">
            <el-option v-for="item in pileOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
...
import {
  ...
  findChargePile,
  getChargePileList,
  UpdateChargePileByIds, UpdateChargePileByIds2
} from '@/api/chargePile'
...
async function OpenPile2() {
  const ids = []
  multipleSelection.value &&
  multipleSelection.value.map(item => {
    ids.push(item.ID.toString())
  })
  await UpdateChargePileByIds2({
    mode: 'SWITCHON',
    switch_array: ids
  })
  getTableData()
